...
run_frk<-function(nres){
  ...
  test_data_resp<-test_data$temp
  test_data$temp<-NA
  
  data<-rbind(train_data,test_data)
  
  coordinates(train_data)  <- ~east + north        # Convert to SpatialPointsDataFrame
  ...
  fitting_time <- system.time({
    ...
    coordinates(BAUs)  <- ~east+north        # convert to SpatialPointsDataFrame
    gridded(BAUs) <- TRUE  
    ...
    ## Estimate using ML
    S <- FRK(f = temp~1+east+north ,                       # formula for SRE model
  ...
  # Predict on test set
  coordinates(test_data)<- ~east+north 
  pred_test_time<-system.time(pred_test <- predict(S, newdata = test_data))[3]
  
  #add nugget to obtain variances for y
  ...
  test_rmse<-sqrt(mean((test_data_resp - pred_test$mu)^2))
  
  #log_score
  train_score<-mean( (0.5*(pred_train$mu-train_data$temp)^2)/pred_train$var + 
                       0.5*log(2*pi*pred_train$var) )
  test_score<-mean( (0.5*(pred_test$mu-test_data_resp )^2)/pred_test$var + 
