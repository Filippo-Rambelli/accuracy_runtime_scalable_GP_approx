...
#load data
train_data = pd.read_csv("data/MODIS_data_train.txt", delim_whitespace=True, usecols=[1, 2, 3], header=0, names=['east', 'north', 'temp'])
test_data = pd.read_csv("data/MODIS_data_test.txt", delim_whitespace=True, usecols=[1, 2, 3], header=0, names=['east', 'north', 'temp'])

y_train = train_data['temp']
coords_train = train_data[['east', 'north']]

y_test = test_data['temp']
coords_test = test_data[['east', 'north']]

#covariance parameters estimated by a Vecchia approximation with 240 neighbours
truth = np.array([1.289960802320046784e-01,5.637642490377192672e+00,5.033750011756336789e+03])
#coefficients estimated by a Vecchia approximation with 240 neighbours  
coefs = np.array([-5.039085196489946128e+01,-8.039247275051138457e-06,4.401559178540987520e-06])
...
def gpboost_run(gp_approx, **kwargs):
    ...
    filename = f"results/modis23/{base_name}_modis23_{param_str}".replace("__", "_")
