...
X_train <- house_train[, !(colnames(house_train) %in% c("log_price"))]
X_inter <- house_inter[, !(colnames(house_inter) %in% c("log_price"))]
X_extra <- house_extra[, !(colnames(house_extra) %in% c("log_price"))]

#covariance parameters estimated via exact calculations
range=0.0603076839309954
nugget=0.05513030716341601
marginal_var=0.20188955237350595

#regression coefficients estimated via exact calculations
coefs<-c(12.614722511788656,-0.12436141170281728,-0.023595011562458013,-0.11568609738546891,0.5469768881001963,0.016115405756767467,0.1310218698487707,
...
run_mra_with_covs<-function(r){
  ...
  fitting_time <- system.time({
    vecchia.est<- vecchia_estimate_fixed_smoothness_1.5(
      ...
      X = as.matrix(X_train),conditioning="mra", mra.options = mra.options.mra,reltol = 1e-6)
  ...
  #detrend train
  trend<-as.matrix(X_train)%*%coefs
  train_detrended<-house_train$log_price-trend
  ...
  pred_inter_time <- system.time({
    pred_inter <- vecchia_pred(
      vecchia.est, as.matrix(house_inter[,1:2]),X.pred=as.matrix(X_inter),
      mra.options = mra.options.mra, conditioning = 'mra', verbose = TRUE
    )
  })[3]
  ...
  pred_extra_time <- system.time({
    pred_extra <- tryCatch({
      vecchia_pred(
        vecchia.est, 
        as.matrix(house_extra[, 1:2]),
        X.pred = as.matrix(X_extra),
  ...
  inter_rmse<-sqrt(mean((house_inter$log_price - pred_inter$mean.pred)^2))
  
  #log_score
  train_score<-mean( (0.5*(pred_train$mu.obs-house_train$log_price)^2)/pred_train$var.obs +
                       0.5*log(2*pi*pred_train$var.obs) )
  inter_score<-mean( (0.5*(pred_inter$mean.pred-house_inter$log_price)^2)/pred_inter$var.pred +
  ...
  inter_crps<-mean(crps(list(mean=pred_inter$mean.pred,sd=sqrt(pred_inter$var.pred)),
                        house_inter$log_price))
  
  #rmse between predictive means
  train_rmse_mean<-sqrt(mean((pred_train$mu.obs-pred_exact_mean_train)^2))
  inter_rmse_mean<-sqrt(mean((pred_inter$mean.pred-pred_exact_mean_inter)^2))
  ...
  inter_kl<-compute_kl(pred_exact_var_inter,pred_inter$var.pred,pred_exact_mean_inter,pred_inter$mean.pred)
  
  #extrapolation
  if (is.null(pred_extra)) {
    message("Skipping metrics for extrapolation prediction due to failure.")
    ...
    extra_rmse<-sqrt(mean((house_extra$log_price - pred_extra$mean.pred)^2))
    extra_score<-mean( (0.5*(pred_extra$mean.pred-house_extra$log_price)^2)/pred_extra$var.pred +
                         0.5*log(2*pi*pred_extra$var.pred) )
    extra_crps<-mean(crps(list(mean=pred_extra$mean.pred,sd=sqrt(pred_extra$var.pred)),
                          house_extra$log_price))
    extra_rmse_mean<-sqrt(mean((pred_extra$mean.pred-pred_exact_mean_extra)^2))
    extra_rmse_var<-sqrt(mean((pred_extra$var.pred-pred_exact_var_extra)^2))
    extra_kl<-compute_kl(pred_exact_var_extra,pred_extra$var.pred,pred_exact_mean_extra,pred_extra$mean.pred)
  }
  
  # Create the filename
  filename <- paste0("results/house_with_covs/mra_house_with_covs_",r)
