...
#package GPVecchia
vecchia_estimate_given_smoothness <- function(data, locs, X, m = 20, nu,
  ...
  negloglik.vecchia <- function(lgparms) {
    ...
    full_params <- c(full_params[1:2], nu, full_params[3]) # Insert the fixed smoothness value
  ...
  theta.hat <- c(theta.hat[1:2], nu, theta.hat[3]) # Fix smoothness to specific value
...
run_mra<-function(range=0.2,nu_sim = 1.5, nu_estim = 1.5, nugget = 0.5,r=2){
  set.seed(r)
  ...
  for (i in 1:nrep){
    ...
    fitting_times[i] <- system.time({
      vecchia.est <- vecchia_estimate_given_smoothness(
        data = train$y,
    ...
    tryCatch({
      true_negloglik_eval_times[i] <- system.time({
        true_negloglik_list[[i]] <- -vecchia_likelihood(train$y, vecchia.approx, 
                                                        c(marginal_var, eff_range, nu_estim), nugget)
      })[3]
    }, error = function(e) {
    ...
    tryCatch({
      wrong_negloglik_eval_times[i] <- system.time({
        wrong_negloglik_list[[i]] <- -vecchia_likelihood(train$y, vecchia.approx,  
                                                        c(2 *marginal_var, 2 *eff_range, nu_estim), 2 * nugget)
      })[3]
    }, error = function(e) {
      message("Error at iteration ", i, ": ", e$message)
    ...
    if(i<=nrep/2){
      ...
      pred_train_times[i] <- system.time({
        pred_train <- vecchia_prediction(
          train$y, vecchia.approx, c(marginal_var, eff_range, nu_estim), nugget
        )
      })[3]
      ...
      pred_inter_times[i] <- system.time({
        ...
        pred_inter <- vecchia_prediction(train$y, vecchia.approx, 
                                         c(marginal_var, eff_range, nu_estim), nugget)})[3]
  ...
  if (nu_estim != nu_sim) {
    filename_save <- sprintf("results/%s_100k_misspecified/mra_%s_100k_misspecified_%s",
                             range, range, param_str)
  } else if (nugget != 0.5) {
    filename_save <- sprintf("results/%s_100k_n%s/mra_%s_100k_n%s_%s",
                             range, nugget, range, nugget, param_str)
  } else if (nu_sim != 1.5) {
    filename_save <- sprintf("results/%s_100k_s%s/mra_%s_100k_s%s_%s",
                             range, nu_sim, range, nu_sim, param_str)
  } else {
    filename_save <- sprintf("results/%s_100k/mra_%s_100k_%s",
                             range, range, param_str)
  }
...
#example usage
run_mra(range=0.2, nu_sim = 1.5, nu_estim = 1.5, nugget = 0.5, r=1)